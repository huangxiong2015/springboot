...
//创建上传(必须上传)
function createUploader(browseButtonId,fileSize,ida,idaa, callback) {
			...
			FileUploaded : function(up, file, info) {
				if (info.status == 200 || info.status == 203) {
					...
					if(!imageUrl || imageUrl.length <= 0||imageUrl.toLowerCase().indexOf('.pdf')>0){
						var url = params["url"];
						$("#"+ida)[0].setAttribute("data-urlPreview",imageUrlPreview);
						$("#"+ida)[0].setAttribute("data-url",imageUrl);
						$("#"+ida)[0].setAttribute("data-pdfname",file.name);
					}else{
						image = params["image"];// 原图url
						
						/*var _img = $("#"+ida)[0]; //原图image
						_img.src = imageUrlPreview;
						_img.setAttribute("data-url",imageUrl);*/
						/*_img.setAttribute("data-urlPreview",imageUrlPreview);*/
						$("#"+ida)[0].setAttribute("data-urlPreview",imageUrlPreview);
						$("#"+ida)[0].setAttribute("data-url",imageUrl);
						$("#"+ida)[0].setAttribute("data-pdfname",file.name);
						$("#"+ida)[0].src = imageUrlPreview;
						
						var _img2 = $("#"+idaa)[0]; //原图image--大图
						_img2.src = imageUrlPreview;
