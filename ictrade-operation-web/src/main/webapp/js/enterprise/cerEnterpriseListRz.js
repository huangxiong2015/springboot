				...
				data : {
					...
					},
					{
						...
						items: [ {
							...
							target: '_blank',
							show : true,
							href: ykyUrl._this + '/certificationEntRz.htm?action=detail&id={id}',
                            target: '_blank'
						}, {
							className : 'btn-edit',
							show : true,
							text : '编辑',
							href: ykyUrl._this + '/certificationEntRz.htm?action=edit&id={id}',
                            target: '_blank'
						}, {
							className : 'btn-invalid',
							show : "'{activeStatus}' != 'INVALID'",
							text : '失效',
							callback: {
								action: 'invalid',
								params: ['{id}']
...
//操作-失效
function invalid(index, params) {
	
	var _html = "<div class='y_prompt_content_des' >"+
		"<p>确定失效认证企业？</p>" +
		"<p>失效后，该企业所有账号将无法享受企业专享特权。<br/>请填写失效原因：</p>"+
		"<p><textarea type='text' class='w340' id='description' class='description' placeholder='请填写账号失效原因（50字以内）' name='description'  maxlength='100'></textarea></p>"+
	...
	var l = layer.confirm(_html, {
		...
		yes:function(){
			...
			if(reason == ""){
				$("#description").css('border-color', '#b1191a');
				return;
			}
			
			var url = ykyUrl.party + '/v1/enterprises/invalidAccount/'+ params[0] +"?reason="+reason;
	
			syncData(url, 'PUT', null, function() {
				layer.close(l);
		    	location.reload()
			})
		}
