	...
	vm = new Vue({
		...
		created(){
			...
			if(this.brandId == ''){
				...
				this.getRelevantData(this.brandId,false,'hot');
				this.getRelevantData(this.brandId,false,'latest');
			...
			syncData(ykyUrl.product + '/v1/products/manufacturers','GET',{size:5000},function(res,err){
				if(err == null){
					that.brandSelect.options = res.list;
				}
			})			
			syncData(ykyUrl.party + "/v1/party/allparty", 'PUT',{roleType:"SUPPLIER",status:"PARTY_ENABLED"}, function(res,err) {   //制造商数据请求
				if (res) {
		...
		methods : { 
			...
			getVendorSelected:function(ele){
				...
				this.brandSelect.selected = String(ele.id);
				this.showModal = false;
				this.showFoot = true;
				this.showVendor = false;
				this.getRelevantData(ele.id,true,'hot');
			...
			getRelevantData:function(id,isAsync,type){//获取代理线、分类等数据
				var that = this;
				if(type=='hot'){
					syncData(ykyUrl.product + '/v1/products/findProductInfos','GET',{arg:id,flag:'b'},function(res,err){
						if(err == null){
							...
							//新增的时候自动拉取前5条热销型号
							if(that.brandId == ''&&arr.length>0&&that.hotList.datas.length==0){
					...
					syncData(ykyUrl.product + '/v1/products/findNewProducts','GET',{arg:id,flag:'b'},function(res,err){
						if(err == null){
							var arr = [];
							$.each(res,function(i,e){
								var obj = {
							...
							//新增的时候自动拉取前5条最新型号
							if(that.brandId == ''&&arr.length>0&&that.newestList.datas.length==0){
