/**
 * create by tb.zhoulixia@yikuyi.com
 * time: 2017.7.20
 */
var vm = new Vue({
	el: '#activity-upload',
	...
	methods:{
		...
		backPrev: function(){
			location.href = ykyUrl._this + '/shoppromotion/information/'+vm.activityId+'.htm?from=upload'
		},
		toNext: function(){
			syncData(ykyUrl.product + "/v1/activities/"+$("#activityId").val()+"/periods", 'PUT', null, function(res,err) {
				if (!err) {
					location.href = ykyUrl._this + '/shoppromotion/maintain/'+vm.activityId+'.htm';
				}else{
					return;
				}
			});		
		},
		multipleDelete:function(prdId){
			var that = this;
			var ids = [], checkedItem;
			if(prdId){
				ids.push(prdId);
...
var getDataList = function(){
	...
	vm.pageflag = true;
	vm.refresh = true;
	vm.showPagesizeChange = true;
};

var deleteItem = function(index, params){
	vm.multipleDelete(params[0]);
};
