/**
 * create by roy.he@yikuyi.com at 2017/11/07
 */
	...
	data:{
		...
		gridColumns : [{
			...
			items: [{
				...
				text:'标准化',
				show: '{status}==1',
				href: ykyUrl._this+'/productCategory.htm',
				target: '_blank'
			},{
				className: 'btn-detail',
				text:'更新',
	...
	created: function(){
		...
		syncData(ykyUrl.party + '/v1/party/partygroups', 'PUT', {}, function(data,err){
			if(!err){
				...
				_this.formData.fields[3].options = validData;
			}
		})
	},
	methods: {
		...
		if (!err) {
			if(res && res > 0){
				...
				layer.open({
	        		...
	        		move: false,
	        		skin: "s_select_data",
	        		content: '<div style="padding: 15px 20px;"><p style="font-weight:700;font-size: 14px;">更新失败，未找到匹配数据</p><p>请先至类别管理中进行维护</p></div>',
	        		btn: ['去维护','取消'],
	        		yes: function(index, layero){
	        			window.location.href=ykyUrl._this + '/aliasCategory.htm'
	        		},
...
function formatCategory(index, params){
	var val = '';
