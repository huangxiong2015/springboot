/**
 * create by roy.he@yikuyi.com at 2017-7-20
 */
	...
	data: {
		...
		gridColumns : [{
			...
			items: [ {
				...
				show : true,
				target: '_blank',
				href : ykyUrl._this + '/coupon/detail.htm?id=' + '{couponId}'
			}, {
				className : 'btn-detail ml5',
				show : "'{statusId}' !== 'END' && ('{couponCate}' == 'PLATFORM_PROMO')&&('{statusId}' == 'WAIT_SEND')",
				text : '发放',
				callback: {
					action : 'changeStatus',
					params : [ '{couponId}']
				...
				className : 'btn-detail ml5',
				show : "'{statusId}' !== 'END' && ('{couponCate}' == 'EXACT_PROMO')&&('{totalQty}' - '{sendQty}')",
				text : '发放',
				callback: {
					action : 'popHandleFrame',
					params : [ '{couponId}']
				}
			},{
				className : 'btn-detail ml5',
				show : "'{statusId}' !== 'END' && ('{couponCate}' == 'OFFLINE_PROMO')&&('{totalQty}' - '{sendQty}')",
				text : '发放',
				target: '_blank',
				href: ykyUrl._this + '/coupon.htm?action=groundhandleOut&&id=' + '{couponId}'
			},{
				className : 'btn-detail ml5',
				show : "'{statusId}' !== 'END' && ('{couponCate}' == 'EXACT_PROMO')&&!('{totalQty}' - '{sendQty}')",
				text : '发放',
...
function qtyNull(){
	layer.msg("该优惠券已领取完，如需继续发放，请联系优惠券创建员修改发行量")
}
...
function changeStatus(index, params){
	var inputLayer = layer.open({
		...
		area: ['620px', '200px'],
		skin:"up_skin_class",
		content: "<div class='set_sending_layer'><h4>确定发放后优惠券可领取或自动发放到用户账号</h4></div>",
		yes:function(){
			setCouponReady(params[0],inputLayer);
		},
		cancel: function(){
