...
$(function() {
								...
								function(data, err) {
									if(err==null){
										if((vm.activityId && !vm.from) || (vm.activityId && vm.from=='maintain')){
											location.href = ykyUrl._this + '/timepromotion/maintain/' + data.activityId + '.htm?from=information';
										}else{
											location.href = ykyUrl._this + '/timepromotion/upload/' + data.activityId + '.htm';
										}
	...
	$('.btn-concle').on('click', function(){		
		window.location.href = operationWebUrl + "/timepromotion/list.htm"; 					
		});
});
