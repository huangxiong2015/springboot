...
public interface IMovQueryResourceV2 {
	...
	@ApiOperation(value = "批量校验MOV信息", notes = "作者：王洪<br/>批量校验MOV信息" ,response = MovValidResult.class,responseContainer = "List")
	@ApiResponses(value ={@ApiResponse(code = 404, message = "访问的页面不存在", response = Void.class) })
	@RequestMapping(method=RequestMethod.POST)
	public List<MovValidResult> validMov(@ApiParam("MOV校验参数实体")List<MovValidationParam> movValidationParams)throws BusinessException;
}
